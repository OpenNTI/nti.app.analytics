...
class TestUsageStats( NTIAnalyticsTestCase ):
	...
	def test_resource_usage_stats(self, mock_events, mock_get_title,
		...
		events = []
		avg_session_time = ':24' # 120s divided by 5
		avg_watch_time = ':12' # 120s divided by 10 students
	...
	def test_video_usage_stats(self, mock_events, mock_get_title,
		...
		single_resource_id = 'ntiid1'
		video_duration = 40
		event = MockVideoEvent( 'Public1', single_resource_id, 20, 1, video_duration, 40 )
		mock_events.is_callable().returns( (event,) )
		video_stats = self.get_video_stats()
		results = video_stats.get_stats()
		assert_that( results, has_length( 1 ) )
		...
		avg_session_time = ':%s' % ( int( total_watch_time / session_count ) )
		avg_watch_time = ':%s' % ( int( total_watch_time / 10 ) )
		# 2 students out of 10 watched completely: Public1 in aggregate, ForCredit3 in one event
		percentage_watched_completely = '20%'
